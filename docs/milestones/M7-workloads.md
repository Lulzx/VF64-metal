...
SpMV, GEMV, GEMM, batched two-variable LP vertex enumeration, and a direct
N-body force evaluation. The cross-mode kernels contain no CPU arithmetic
...
including 7.38x CPU for `fast48` GEMM at 41.28 p01 accuracy bits and 3.71x CPU
...
This is not the M7 exit. CuMetal CUDA workloads, energy measurements, a matched
CPU GMRES timing baseline, multi-step N-body conservation evidence, broader LP
and sparse matrices, and cross-device reproduction remain open.
