...
| M7 workloads | In progress | M4 Pro pilot covers CG, GMRES, three CSR structures, GEMV, GEMM, four structured LP families, multi-step N-body, plus the legacy CuMetal CUDA pair contract | VF64-integrated CuMetal CUDA path, energy, general sparse LP validation, external sparse matrices, and cross-device reproduction |
